...
namespace osuCrypto
{
    ...
    void KkrtNcoOtSender::setUniformBaseOts(
    ...
    coproto::Proto KkrtNcoOtSender::init(
        u64 numOTExt, PRNG& prng)
    {
        ...
        struct Proto : public coproto::NativeProto
        {
            ...
            u64 numOTExt;
            PRNG& prng;
            Proto(KkrtNcoOtSender& o, u64 n, PRNG& p)
                :ot(o)
                , numOTExt(n)
                , prng(p)
            {}
...

                {
                    ...
                    for (u64 superBlkIdx = 0; superBlkIdx < numSuperBlocks; ++superBlkIdx)
                    {
                        ...
                        for (u64 i = 0; i < numCols / 128; ++i)
                        {
                            ...
                            {
                                ot.mGens[colIdx].ecbEncCounterMode(ot.mGensBlkIdx[colIdx], superBlkSize, ((block*)t.data() + superBlkSize * tIdx));
...
#define KKRT_WIDTH 4
    ...
    coproto::Proto KkrtNcoOtSender::recvCorrection(u64 recvCount)
    {
        ...
        // there c(w) is a pseudo-random code.
        auto dest = mCorrectionVals.begin() + (mCorrectionIdx * mCorrectionVals.stride());
        auto data = coproto::span<u8>((u8*)&*dest, recvCount * sizeof(block) * mCorrectionVals.stride());
