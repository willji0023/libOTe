...
namespace osuCrypto
{
    ...
    coproto::Proto KosOtExtReceiver::setBaseOts(span<std::array<block, 2>> baseOTs, PRNG& prng)
    {
        struct Proto : public coproto::NativeProto
        {
            ...
            PRNG& prng;
            Proto(KosOtExtReceiver& o, span<std::array<block, 2>> m, PRNG& p)
                :ot(o)
                , messages(m)
                , prng(p)
            {}
            ...
            coproto::error_code resume() override
            {
                ...
                if (messages.size() != gOtExtBaseOtCount)
                ...
                for (u64 i = 0; i < gOtExtBaseOtCount; i++)
                {
                    ot.mGens[i][0].SetSeed(messages[i][0 ^ *iter]);
                    ot.mGens[i][1].SetSeed(messages[i][1 ^ *iter]);
        ...
        return coproto::makeProto<Proto>(*this, baseOTs, prng);
    }
    ...
    coproto::Proto KosOtExtReceiver::receive(
        ...
        PRNG& prng)
    {
        struct Proto : coproto::NativeProto
        {
            ...
            u64 numOtExt, numSuperBlocks, numBlocks, superBlkIdx;
#ifdef OTE_KOS_FIAT_SHAMIR
            RandomOracle fs;
#else
            block seed, theirSeed;
            Commit myComm;
#endif
            BitVector choices2;
            ...
            block* xIter, *uIter, *uEnd, * tIter, *cIter;
            span<block>::iterator mIter;
            u64 step;
            std::vector<block> uBuff;
            std::vector<block> correlationData;
            ...
            coproto::error_code resume() override
            {
                ...
                ot.setTimePoint("Kos.recv.start");
                ...
                choices2.reserve(numBlocks * 128);
                ...
                for (superBlkIdx = 0; superBlkIdx < numSuperBlocks; ++superBlkIdx)
                {
                    ...
                    for (u64 colIdx = 0; colIdx < 128; ++colIdx)
                    {
                        ...
                        ot.mGens[colIdx][0].mAes.ecbEncCounterMode(ot.mGens[colIdx][0].mBlockIdx, superBlkSize, tIter);
                        ot.mGens[colIdx][1].mAes.ecbEncCounterMode(ot.mGens[colIdx][1].mBlockIdx, superBlkSize, uIter);
                ...
                ot.setTimePoint("Kos.recv.transposeDone");
...
#error "OTE_KOS_HASH" must be defined
        ...
        return coproto::makeProto<Proto>(*this, choices, messages, prng);
