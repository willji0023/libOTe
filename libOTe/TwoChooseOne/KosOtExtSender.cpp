...
namespace osuCrypto
{
    ...
    coproto::Proto KosOtExtSender::setBaseOts(span<block> baseRecvOts, const BitVector & choices)
    {
        struct Proto : public coproto::NativeProto
        {
            ...
            BitVector delta;
            span<u8> dd;
            Proto(KosOtExtSender& o, span<block> b, const BitVector& c)
            ...
            coproto::error_code resume() override
            {
                ...
                delta.resize(128);
                dd = delta.getArrayView<u8>();
                CP_RECV(dd);

                ot.setUniformBaseOts(baseRecvOts, choices ^ delta);
                CP_END();
                return {};
            }
        };
    ...
    coproto::Proto KosOtExtSender::send(
        ...
        PRNG& prng)
    {
        struct Proto : public coproto::NativeProto
        {
            ...
            span<std::array<block, 2>> messages;
            PRNG& prng;
            Proto(KosOtExtSender& o, span<std::array<block, 2>>m, PRNG& p)
                : ot(o)
                , messages(m)
                , prng(p)
            {}
...
#error "OTE_KOS_HASH" must be defined
            ...
            coproto::error_code resume() override
            {
                ...
                ot.setTimePoint("Kos.send.start");
                ...
                for (superBlkIdx = 0; superBlkIdx < numSuperBlocks; ++superBlkIdx)
                {
                    ...
                    if (uIter == uEnd)
                    {
                        ...
                        size = step * superBlkSize * 128 * sizeof(block);
                        data = span<u8>((u8*)u.data(), size);
...
#endif
                ot.setTimePoint("Kos.send.transposeDone");
#ifdef OTE_KOS_FIAT_SHAMIR
                block seed;
                fs.Final(seed);
                ...
                ot.setTimePoint("Kos.send.cncSeed");
                ...
                doneIdx = 0;
                ot.setTimePoint("Kos.send.checkStart");

                end = (messages.size() + 127) / 128;
                ...
                ot.setTimePoint("Kos.send.proofReceived");
                {

                    block& received_x = proof[0];
                    block& received_t = proof[1];
                    block& received_t2 = proof[2];
